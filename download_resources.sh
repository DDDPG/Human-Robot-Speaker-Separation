...
if [[ "$sample_rate" == "8k" || "$sample_rate" == "all" ]]; then
    ...
    if [[ "$version" == "raw" || "$version" == "all" ]]; then
        ...
        wget -O https://archive.org/download/hrsp-2mix-8k-raw/HRSP2mix_8k_raw.zip
        tar -xzf temp_8k_raw.tar.gz -C /HRSP2mix_8k/
        rm temp_8k_raw.tar.gz
    fi
    if [[ "$version" == "clean" || "$version" == "all" ]]; then
        wget -O temp_8k_clean.tar.gz https://archive.org/download/hrsp-2mix-8k-clean/HRSP2mix_8k_clean.zip
...
if [[ "$sample_rate" == "16k" || "$sample_rate" == "all" ]]; then
    ...
    if [[ "$version" == "raw" || "$version" == "all" ]]; then
        wget -O temp_16k_raw.tar.gz https://archive.org/download/hrsp-2mix-16k-raw/HRSP2mix_16k_raw.zip
        tar -xzf temp_16k_raw.tar.gz -C HRSP2mix_16k/
        rm temp_16k_raw.tar.gz
    fi
    if [[ "$version" == "clean" || "$version" == "all" ]]; then
        wget -O temp_16k_clean.tar.gz https://archive.org/download/hrsp-2mix-16k-clean/HRSP2mix_16k_clean.zip
